...
impl<T: Send + 'static> LazyHandle<T> {
    ...
    pub(crate) fn new(rx: oneshot::Receiver<T>) -> Self {
        Self { inner: Arc::new(LazyHandleInner { rx: std::sync::Mutex::new(Some(rx)), value: OnceLock::new() }) }
    }

    /// Creates a handle that is already resolved with the given value.
    pub fn ready(value: T) -> Self {
        let inner = LazyHandleInner { rx: std::sync::Mutex::new(None), value: OnceLock::from(value) };
    ...
    pub fn get(&self) -> &T {
        self.inner.value.get_or_init(|| {
            ...
            rx.blocking_recv()
                .expect("LazyHandle task dropped without producing a value")
        })
    }
