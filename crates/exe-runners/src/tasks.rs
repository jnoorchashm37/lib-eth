...
///
/// The main purpose of this type is to abstract over [`TaskExecutor`] so it's
/// more convenient to provide default impls for testing.
///
///
/// # Examples
///
...
/// ```
///
/// Use the [`TaskExecutor`] that spawns task directly onto the tokio runtime
/// via the [Handle].
///
/// ```
...
/// ```
///
/// The [`TaskSpawner`] trait is [`DynClone`] so `Box<dyn TaskSpawner>` are also
/// `Clone`.
...
impl TaskSpawner for TokioTaskExecutor {
    ...
    fn spawn_critical_blocking(&self, _name: &'static str, fut: BoxFuture<'static, ()>) -> JoinHandle<()> {
        tokio::task::spawn_blocking(move || tokio::runtime::Handle::current().block_on(fut))
    }
}
...
impl TaskExecutor {
        ...
        F: Future<Output = ()> + Send + 'static
    {
        let panicked_tasks_tx = self.panicked_tasks_tx.clone();
        let on_shutdown =
            GracefulShutdown::new(self.on_shutdown.clone(), GracefulShutdownGuard::new(Arc::clone(&self.graceful_tasks)));
        let fut = f(on_shutdown);
...
mod tests {
    ...
    fn test_manager_graceful_shutdown_many() {
        ...
        for _ in 0..num {
            let c = counter.clone();
            executor.spawn_critical_with_graceful_shutdown_signal("grace", move |shutdown| async move {
                let _guard = shutdown.await;
                tokio::time::sleep(Duration::from_millis(200)).await;
