//! Task execution and CLI runner utilities.
//!
//! # Feature Flags
//!
//! - `rayon`: Enable rayon thread pools and ordered parallel iterator helpers.
//! - `reth-tasks`: Re-export upstream `reth_tasks` instead of the local mirror.
...
///
/// This function captures the current tokio runtime handle (if available) and
/// enters it in the newly spawned thread. This ensures that code running in the
/// spawned thread can use [`Handle::current()`], [`Handle::spawn_blocking()`],
/// and other tokio utilities that require a runtime context.
#[cfg(not(feature = "reth-tasks"))]
#[track_caller]
...
///
/// This is the scoped thread version of [`spawn_os_thread`], for use with
/// [`std::thread::scope`].
#[cfg(not(feature = "reth-tasks"))]
#[track_caller]
...
/// panicked, for diagnostic purposes, since tokio tasks essentially fail
/// silently. Therefore, this type is a Future that resolves with the name of
/// the panicked task. See [`Runtime::spawn_critical_task`].
///
/// Automatically spawned as a background task when building a [`Runtime`]. Use
/// [`Runtime::take_task_manager_handle`] to extract the join handle if you need
/// to poll for panic errors directly.
#[cfg(not(feature = "reth-tasks"))]
#[derive(Debug)]
...
#[cfg(not(feature = "reth-tasks"))]
impl TaskManager {
    /// Create a new [`TaskManager`] without an associated [`Runtime`],
    /// returning the shutdown/event primitives for [`RuntimeBuilder`] to
    /// wire up.
    pub(crate) fn new_parts(_handle: Handle) -> (Self, Shutdown, UnboundedSender<TaskEvent>, Arc<AtomicUsize>) {
        ...
        let manager = Self { task_events_rx, signal: Some(signal), graceful_tasks: Arc::clone(&graceful_tasks) };
