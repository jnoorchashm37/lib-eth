...
//!
//! Each worker is a dedicated OS thread that processes closures sent to it via
...
/// A single-thread worker that processes closures sequentially on a dedicated
/// OS thread.
struct WorkerThread {
...
impl WorkerMap {
        ...
        R: Send + 'static
    {
        ...
        let worker = self
            .workers
            .entry(name)
            .or_insert_with(|| WorkerThread::new(name));
...
mod tests {
    ...
    async fn worker_map_same_thread() {
        ...
        let id1 = map
            .spawn_on("test", || thread::current().id())
            .await
            .unwrap();
        let id2 = map
            .spawn_on("test", || thread::current().id())
    ...
    async fn worker_map_different_names_different_threads() {
        ...
        let id1 = map
            .spawn_on("worker-a", || thread::current().id())
            .await
            .unwrap();
        let id2 = map
            .spawn_on("worker-b", || thread::current().id())
            .await
            .unwrap();
