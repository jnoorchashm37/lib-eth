...
impl Default for TokioConfig {
    fn default() -> Self {
        Self::Owned { worker_threads: None, thread_keep_alive: DEFAULT_THREAD_KEEP_ALIVE, thread_name: "tokio-rt" }
    }
}
...
impl TokioConfig {
    ...
    /// Create a config for an owned runtime with the specified number of worker
    /// threads.
...
impl RayonConfig {
    ...
    pub const fn with_proof_storage_worker_threads(mut self, proof_storage_worker_threads: usize) -> Self {
    ...
    pub const fn with_proof_account_worker_threads(mut self, proof_account_worker_threads: usize) -> Self {
...
impl Runtime {
    ...
    /// Spawns a blocking closure directly on the tokio runtime.
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        ...
        let finished_critical_tasks_total_metrics = self.0.metrics.finished_critical_tasks_total.clone();
        let task = async move {
    ...
    /// Spawns a critical task with graceful shutdown support.
    ...
    pub fn initiate_graceful_shutdown(&self) -> Result<GracefulShutdown, tokio::sync::mpsc::error::SendError<()>> {
        self.0
            .task_events_tx
            .send(TaskEvent::GracefulShutdown)
            .map_err(|_send_error_with_task_event| tokio::sync::mpsc::error::SendError(()))?;

        Ok(GracefulShutdown::new(self.0.on_shutdown.clone(), GracefulShutdownGuard::new(Arc::clone(&self.0.graceful_tasks))))
    }
...
mod tests {
    ...
    fn test_runtime_config_existing_handle() {
        ...
        let config = Runtime::test_config().with_tokio(TokioConfig::existing_handle(rt.handle().clone()));
