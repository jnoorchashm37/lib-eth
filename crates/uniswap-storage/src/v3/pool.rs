...
pub async fn v3_tick_data<P: StorageSlotFetcher>(
    ...
    let liquidity_net_i128 = (((slot0_data >> 128) & U256::from(u128::MAX)) as U256).to::<u128>() as i128;
...
mod tests {
    ...
    async fn test_v3_current_tick() {
        ...
        let result = v3_current_tick::<RootProvider>(&provider, POOL_ADDRESS, Some(BlockId::number(block_number)))
            .await
            .unwrap();
    ...
    async fn test_v3_current_liquidity() {
        ...
        let result = v3_current_liquidity::<RootProvider>(&provider, POOL_ADDRESS, Some(BlockId::number(block_number)))
            .await
            .unwrap();
    ...
    async fn test_v3_fee_growth_global() {
        ...
        let result = v3_fee_growth_global::<RootProvider>(&provider, POOL_ADDRESS, Some(BlockId::number(block_number)))
            .await
            .unwrap();
        let expected = (
    ...
    async fn test_v3_sqrt_price_x96() {
        ...
        let result = v3_sqrt_price_x96::<RootProvider>(&provider, POOL_ADDRESS, Some(BlockId::number(block_number)))
            .await
            .unwrap();
    ...
    async fn test_v3_tick_data() {
        ...
        let result = v3_tick_data::<RootProvider>(&provider, POOL_ADDRESS, tick, Some(BlockId::number(block_number)))
            .await
            .unwrap();
        let expected = TickData {
            ...
            fee_growth_outside0_x128: U256::from_str_radix("3114302369834678599345866169150924", 10).unwrap(),
            fee_growth_outside1_x128: U256::from_str_radix("1399568734137934671216963164240387940486177", 10).unwrap()
        };
